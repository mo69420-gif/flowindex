...
export default function Scenarios() {
  ...
  return (
    <TerminalLayout
      title="SCENARIOS"
      syslog={totalOps === 0 ? 'No operations on record. Scan a room first.' : `${totalOps} operation(s) logged. ${totalTargets} total targets neutralized.`}
    >
      {/* Lifetime stats */}
      <div className="border border-border bg-muted p-3 mb-3.5">
        ...
        <div className="space-y-1">
          ...
          <StatRow label="TOTAL TRASH" value={String(totalTrash)} />
          <StatRow label="TOTAL LOOT" value={String(totalLoot)} />
        </div>
      </div>
        ...
        history.map((record, i) => (
          <div key={i} className="border border-border bg-muted p-3 mb-2.5">
            ...
            <div className="grid grid-cols-2 gap-x-4 gap-y-0.5 text-[11px]">
              ...
              <StatRow label="TRASH" value={String(record.trash)} small />
              <StatRow label="LOOT" value={String(record.loot)} small />
            </div>
